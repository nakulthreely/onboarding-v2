...
const Application: React.FC<ApplicationProps> = ({
  ...
  // authenticate url. ensure that only one flag is added otherwise defaults to prod url
  let baseAuthUrl = "https://auth.tria.so";
  if (litStaging && !triaStaging) {
    baseAuthUrl = "https://auth-tria-lit.vercel.app"; // New URL based on the litAuthenticate flag
  } else if (triaStaging && !litStaging) {
    baseAuthUrl = "https://auth-tria.vercel.app";
  }
  ...
  const { eventData } = useListenerCO(baseAuthUrl);
  ...
  useEffect(() => {
    ...
    if (eventData?.message?.accountExists === false) {
      ...
      url = `${baseAuthUrl}/signUpUserNameIframe/${eventData?.message?.social}/${eventData?.message?.userId}/${eventData?.message?.username}/${eventData?.message?.token}`;
  ...
  useEffect(() => {
    ...
    if (eventData?.message?.type == "otpLogin") {
      ...
      url = `${baseAuthUrl}/phoneEmailOtp/?&dappName=${dappName}&dappLogo=${logo}&darkMode=${darkMode}`;
      // console.log('url---------->', url)
      setOpenNewFrame(true);
      setFrameUrl(url);
    }
                            ...
                            <div className="">
                              ...
                              >
                                <iframe
                                  ...
                                  src={`${baseAuthUrl}/SocialLoginIframe/?dappName=${dappName}&dappLogo=${logo}&darkMode=${darkMode}&clientId=${clientId}`}
                                  height={"205px"}
                                  width="100%"
                                  loading="eager"
                                />
          ...
          !customWalletButton && (
            ...
            >
              ...
              >
                ...
                <NewWalletButton bgColor={primaryColor || "#7D40FF"} />
              </div>
            </Draggable>
          )}
        {!walletVisible &&
